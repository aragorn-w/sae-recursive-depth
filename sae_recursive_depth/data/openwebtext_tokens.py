"""OpenWebText tokenization + shard cache.

Streams ``Skylion007/openwebtext`` through a model's tokenizer, packs
...
Mirrors ``src.data.pile_tokens`` byte-for-byte except for:
  * dataset id is ``Skylion007/openwebtext`` instead of ``monology/pile-uncopyrighted``
  * shard filename includes an ``__owt__`` segment to disambiguate from Pile shards
  * default ``seq_len`` is 128 rather than 1024

Added 2026-05-09 to support retraining a GPT-2 Small BatchTopK SAE under
the Leask et al. 2025 recipe (300M OpenWebText tokens, seq_len=128), which
requires an OpenWebText-sourced shard with a smaller context.
"""
...
from .pile_tokens import TOKEN_CACHE_DIR, load_token_shard  # noqa: F401
...
def shard_path(model_id: str, n_tokens: int, seq_len: int, seed: int) -> Path:
    ...
    name = f"{safe_model}__owt__n{n_tokens}__seq{seq_len}__seed{seed}.pt"
...
def prepare_token_shard(
    ...
    *,
    ...
    """Tokenize OpenWebText until ``n_tokens`` are produced; save as one shard.

    Output shard: ``(n_seqs, seq_len)`` int32, where ``n_seqs * seq_len >= n_tokens``.
    Sequences are EOS-separated concatenations of openwebtext documents.
    """
    ...
    ds = load_dataset(OPENWEBTEXT_DATASET_ID, split=dataset_split, streaming=True)
    ...
    if seed:
        # `take` after `skip` is fine on streaming datasets; openwebtext
    ...
    if seq_idx < n_seqs:
        # Truncate if OpenWebText somehow ran out (will not happen in practice).
    ...
    torch.save(
        {
            ...
            "seq_len": seq_len,
            "seed": seed,
            "dataset_id": OPENWEBTEXT_DATASET_ID,
        },
        tmp,
    )
    os.replace(tmp, out)
