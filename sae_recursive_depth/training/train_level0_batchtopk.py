...
Handles rows where `level0_source == "train_from_scratch"`,
`level0_arch == "batchtopk"`, and `depth == 0`.
...
def _resolve_prepare_token_shard(dataset: str):
    """Return the prepare_token_shard callable for the requested dataset."""
    if dataset == "pile":
        from sae_recursive_depth.data.pile_tokens import prepare_token_shard
        return prepare_token_shard
    if dataset == "openwebtext":
        try:
            from sae_recursive_depth.data.openwebtext_tokens import prepare_token_shard
        except ImportError as e:
            raise ImportError(
                f"dataset=openwebtext requested but src.data.openwebtext_tokens is not importable: {e!r}"
            ) from e
        return prepare_token_shard
    raise ValueError(f"unknown dataset {dataset!r}; expected 'pile' or 'openwebtext'")
...
def main() -> None:
    with experiment_context(arch_hint="batchtopk") as ctx:
        ...
        # token_budget overrides the per-base-model TOKEN_BUDGET default.
        n_tokens = int(row.get("token_budget") or TOKEN_BUDGET[base_model])
        n_devices = N_DEVICES.get(base_model, 1)
        # seq_len overrides the SEQ_LEN constant for tokenization shards.
        seq_len = int(row.get("seq_len") or SEQ_LEN)
        # dataset selects pile_tokens vs openwebtext_tokens prepare_token_shard.
        dataset = str(row.get("dataset") or "pile")
        prepare_token_shard = _resolve_prepare_token_shard(dataset)
        device = _device()
        ...
        # sae_batch_size overrides the default SAE micro-batch (2048).
        sae_batch = int(row.get("sae_batch_size") or 2048)
        ...
        # adam_beta1/adam_beta2 override Adam's first/second moment decays.
        adam_beta1 = float(row.get("adam_beta1") or 0.9)
        adam_beta2 = float(row.get("adam_beta2") or 0.999)
        # sparsity_penalty adds an L1-style penalty on latents (off when 0.0).
        sparsity_penalty = float(row.get("sparsity_penalty") or 0.0)
        # enable_auxk forces Bussmann auxk dead-latent revival on from step 0
        # (otherwise gated by AUXK_ENABLED sentinel from evaluate_gates.py).
        force_auxk = bool(row.get("enable_auxk") or False)
        from sae_recursive_depth.training.train_meta_sae import (
            AUXK_DEAD_STEPS,
            AUXK_K,
            AUXK_ALPHA,
            _auxk_enabled,
        ...
        opt = torch.optim.Adam(
            sae.parameters(), lr=base_lr, betas=(adam_beta1, adam_beta2), weight_decay=0.0
        )
        ...
        if not skip_training:
            ...
            dead_counter = (
                torch.zeros(width, dtype=torch.int32, device=device)
                if auxk_on
                else None
            )
            with curves_path.open(curves_mode, buffering=1) as cf:
                ...
                if tokens_remaining > 0:
                    for batch in iter_residual_batches(
                        ...
                        if sparsity_penalty > 0.0:
                            # Leask et al. 2025 §A.5 + saprmarks/dictionary_learning
                            # StandardTrainer + bartbussmann/BatchTopK/sae.py all
                            # compute L1 as sum-over-latents, mean-over-batch.
                            loss = loss + sparsity_penalty * latents.abs().sum(dim=-1).mean()
                        if auxk_on:
                            ...
                            if n_dead > 0:
                                ...
                                if total_kept < flat.numel():
                                    ...
                                    auxk_mask = torch.zeros_like(flat)
                                    auxk_mask.scatter_(0, topk.indices, 1.0)
                                    dead_pre = (flat * auxk_mask).reshape(B, n_dead)
