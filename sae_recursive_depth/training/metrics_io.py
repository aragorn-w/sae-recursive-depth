...
Every training entry point writes
``data/artifacts/<experiment_id>/metrics.tsv`` with one header row and one
value row. ``scripts/evaluate_gates.py`` reads from this file to evaluate
decision gates, and the orchestration layer reads columns 1 and 2 (VE and
PW-MCC) via awk for status messages.
...
METRICS_COLUMNS: tuple[str, ...] = (
    "variance_explained",          # col 1 — shell awk reads this
    "pwmcc",                        # col 2 — shell awk reads this
    "mmcs",
    "pwmcc_null_mean",
    "pwmcc_null_std",
