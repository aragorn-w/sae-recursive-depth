...
``load_level0(...)``. Per training rule 6, no module under ``sae_recursive_depth/training/``
...
Routing:
  ...
  "train_from_scratch"                          batchtopk    local checkpoint (base_model→artifact map)
  "train_from_scratch_row"                      batchtopk    local checkpoint by explicit artifact id
    ...
    *,
    ...
    if rev is None:
        raise RuntimeError(
            ...
            f"PINNED_BASE_MODEL_REVISIONS (sae_recursive_depth/training/loaders.py)."
        )
...
def _require_revision(source: str) -> str:
    ...
    if sha is None:
        raise RuntimeError(
            ...
            f"PINNED_REVISIONS (sae_recursive_depth/training/loaders.py). Pin a SHA before "
...
def load_level0(
    ...
    *,
    ...
    ``source_row`` is required when ``level0_source == "train_from_scratch_row"``
    and identifies a specific local artifact id under
    ``experiments/artifacts/<source_row>/checkpoint.pt``. It is the row-level
    ``level0_source_row`` field plumbed through from the caller. For all other
    ...
    if level0_source == "train_from_scratch_row":
        if not source_row:
            raise ValueError(
                "level0_source='train_from_scratch_row' requires a non-empty "
                "level0_source_row on the row (passed as source_row=...). "
                "Got source_row={!r}.".format(source_row)
            )
        return _load_local_batchtopk_by_id(source_row, base_model, layer, site)
...
def _load_local_batchtopk(base_model: str, layer: int, site: Site) -> LoadedSAE:
    ...
    ckpt_path = REPO_ROOT / "data" / "artifacts" / artifact_id / "checkpoint.pt"
...
def _load_local_batchtopk_by_id(
    artifact_id: str,
    base_model: str,
    layer: int,
    site: Site,
) -> LoadedSAE:
    """Locally trained BatchTopK level-0 checkpoint, addressed by artifact id.

    Bypasses the hardcoded ``base_model -> artifact_id`` mapping in
    ``_load_local_batchtopk`` so a meta-SAE row can target a specific level-0
    artifact (e.g. a Leask-recipe variant rather than the default
    ``l0_gpt2_batchtopk``). The base_model still must match what the artifact
    was trained on; mismatches raise ValueError.
    """
    if not artifact_id:
        raise ValueError(
            "_load_local_batchtopk_by_id requires a non-empty artifact_id; "
            "got {!r}.".format(artifact_id)
        )
    ckpt_path = REPO_ROOT / "data" / "artifacts" / artifact_id / "checkpoint.pt"
    if not ckpt_path.exists():
        raise FileNotFoundError(
    ...
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    ckpt_base_model = ckpt.get("base_model")
    if ckpt_base_model is not None and ckpt_base_model != base_model:
        raise ValueError(
            f"base_model mismatch between row and artifact {artifact_id!r}: "
            f"row base_model={base_model!r}, checkpoint base_model="
            f"{ckpt_base_model!r}. Refusing to mix incompatible bases."
        )
    # Same checkpoint schema as _load_local_batchtopk:
    # W_enc: (d_in, n_latents), W_dec: (n_latents, d_in)
    W_enc = ckpt["W_enc"].to(torch.float16)  # (d_in, n_latents)
    W_enc = W_enc.T.contiguous()             # -> (n_latents, d_in)
    W_dec = ckpt["W_dec"].to(torch.float16).contiguous()  # (n_latents, d_in)
    ...
    return LoadedSAE(
        ...
        b_dec=b_dec.to(torch.float16) if b_dec is not None else None,
        arch="batchtopk",
        source=f"train_from_scratch_row:{artifact_id}",
        revision="local",
        extra={"experiment_id": ckpt.get("experiment_id", artifact_id)},
    )
...
def _synthesize_random_gaussian(base_model: str, layer: int, site: Site) -> LoadedSAE:
    ...
    raise RuntimeError(
        ...
        "sae_recursive_depth/training/train_null_sae.py from d_model and sample count. Callers "
