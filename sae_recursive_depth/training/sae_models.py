...
- ``BatchTopKSAE`` (Bussmann et al. 2024, Matryoshka/BatchTopK line,
  ...
  JumpReLU-style per-latent threshold used at inference. Project convention
  uses ``k = 60`` for level-0 Gemma and ``k = 4`` for meta-SAEs; the module
  reads the value from the caller rather than hard-coding it.
...
``decoder_directions()`` that returns the unit-normalized decoder matrix
(meta-SAEs consume parent decoder directions as their input).
"""
