...
Handles rows with ``depth >= 1``. Input is the decoder-direction matrix of
the previous depth.

Meta-SAEs are always BatchTopK with ``k=4``. The ``level0_arch`` field in
EXPERIMENTS.yaml identifies the *parent* SAE's architecture (which
determines how the parent is loaded), not the meta-SAE's architecture.
"""
...
from sae_recursive_depth.training.loaders import load_level0
...
AUXK_SENTINEL = Path("data/AUXK_ENABLED")
...
def _load_parent_decoder(row: dict) -> tuple[torch.Tensor, torch.Tensor, str]:
    ...
    If ``row['parent_subset_spec']`` is set (path to a .pt file containing a
    boolean mask of length n_latents_parent), the matrices are sliced to the
    rows where mask is True before being returned. The mask file may either
    be a bare bool tensor or a dict containing a ``live_mask_strict`` /
    ``live_mask_1e-5`` / ``mask`` key (first match wins).
    """
    depth = int(row["depth"])
    deps = row.get("dependencies") or []

    if depth == 1:
        # Parent is the level-0 SAE identified by `level0_source`. When the
        # row uses `level0_source == "train_from_scratch_row"`, a specific
        # local artifact id is plumbed through `level0_source_row` rather
        # than the default base_model -> artifact mapping in loaders.py.
        loaded = load_level0(
            level0_source=row["level0_source"],
            base_model=row["base_model"],
            layer=int(row["layer"]),
            site=row["site"],
            source_row=row.get("level0_source_row"),
        )
        ...
        W_enc = F.normalize(W_enc, dim=1)
        W_dec = F.normalize(W_dec, dim=1)
        parent_source = f"{loaded.source}@{loaded.revision}"
        W_enc, W_dec, parent_source = _maybe_apply_parent_subset(
            row, W_enc, W_dec, parent_source
        )
        return W_enc, W_dec, parent_source
    ...
    parent_id = deps[0]
    parent_ckpt_path = Path("data/artifacts") / parent_id / "checkpoint.pt"
    ...
    W_enc = F.normalize(W_enc, dim=1)
    W_dec = F.normalize(W_dec, dim=1)
    parent_source = f"local:{parent_id}"
    W_enc, W_dec, parent_source = _maybe_apply_parent_subset(
        row, W_enc, W_dec, parent_source
    )
    return W_enc, W_dec, parent_source
...
_SUBSET_MASK_KEYS = ("live_mask_1e-5", "live_mask_strict", "mask", "live_mask")


def _maybe_apply_parent_subset(
    row: dict,
    W_enc: torch.Tensor,
    W_dec: torch.Tensor,
    parent_source: str,
) -> tuple[torch.Tensor, torch.Tensor, str]:
    """Slice (W_enc, W_dec) to the True rows of a mask referenced by row.

    If ``row['parent_subset_spec']`` is unset/empty, returns the inputs
    unchanged. The default un-sliced path is therefore preserved.
    """
    spec = row.get("parent_subset_spec")
    if not spec:
        return W_enc, W_dec, parent_source
    ...
    if not spec_path.exists():
        raise FileNotFoundError(
            f"parent_subset_spec missing: {spec_path}. Row {row.get('id')!r} "
            f"references a mask file that does not exist."
        )
    blob = torch.load(spec_path, map_location="cpu", weights_only=False)
    if torch.is_tensor(blob):
        mask = blob
    elif isinstance(blob, dict):
        mask = None
        for k in _SUBSET_MASK_KEYS:
            if k in blob and torch.is_tensor(blob[k]):
                mask = blob[k]
                break
        if mask is None:
            raise ValueError(
                f"parent_subset_spec {spec_path} dict has no recognized mask key "
                f"(tried {_SUBSET_MASK_KEYS!r}); got {list(blob.keys())!r}"
            )
    else:
        raise TypeError(
            f"parent_subset_spec {spec_path} must be a tensor or dict, got {type(blob)}"
        )
    mask = mask.to(torch.bool).flatten()
    if mask.shape[0] != W_dec.shape[0]:
        raise ValueError(
            f"parent_subset_spec mask length {mask.shape[0]} does not match "
            f"parent decoder rows {W_dec.shape[0]} (file: {spec_path})"
        )
    n_kept = int(mask.sum().item())
    if n_kept == 0:
        raise ValueError(f"parent_subset_spec {spec_path} keeps zero rows")
    W_enc_sub = W_enc[mask].contiguous()
    W_dec_sub = W_dec[mask].contiguous()
    parent_source = f"{parent_source}|subset={spec_path.name}@{n_kept}/{mask.shape[0]}"
    return W_enc_sub, W_dec_sub, parent_source
