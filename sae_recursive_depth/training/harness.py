...
Centralizes the boilerplate every training entry point repeats:
  - `wandb.init` is called for every real training body; scaffold stubs
    do not create cloud runs.
  - `results_io.append_result` is invoked in a `finally:` block so a
    partial result is recorded even on interrupt.
  - `set_all_seeds(seed)` runs before any model/data code.
  - artifacts land under `data/artifacts/<experiment_id>/`.
...
the process with code 99 to signal scaffold-only.
"""
...
from sae_recursive_depth.training.config import ExperimentNotFound, load_row
from sae_recursive_depth.training.results_io import append_result
from sae_recursive_depth.training.seed import set_all_seeds
...
def experiment_context(arch_hint: str) -> Iterator[ExpContext]:
    ...
    process with code `EXIT_SCAFFOLD_STUB` to signal scaffold-only.
    """
    args = parse_args()
