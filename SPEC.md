...
while True:
    ...
    log_update(next_exp, result, gate_outcome)
    save_state(state)
    sleep(5)
```
...
- Excludes rows with `conditional.stretch_if_time_permits == true` unless all non-stretch experiments are complete AND the project is at least 4 days from its hard end date.
...
- `train_from_scratch` with `level0_arch == "batchtopk"` and `depth == 0` -> `python -m sae_recursive_depth.training.train_level0_batchtopk`
- `flat_sae_on_activations` -> `python -m sae_recursive_depth.training.train_flat_sae`
- `random_gaussian` -> `python -m sae_recursive_depth.training.train_null_sae`
- any huggingface id with `depth >= 1` -> `python -m sae_recursive_depth.training.train_meta_sae`
- id `autointerp_all` -> `python -m sae_recursive_depth.analysis.run_autointerp`
- id `simplestories_stretch` -> `python -m sae_recursive_depth.training.train_level0_batchtopk` with the simplestories config
...
- `halt_and_notify`: set `status` of the runner in `state.json` to `halted` and exit the loop. Requires human intervention to restart.
...
### 2.3 Logging
Every gate evaluation writes a row to `data/gates.tsv` with columns: `timestamp, experiment_id, metric, observed_value, threshold, action, triggered`. This is append-only.

## 3. Failure recovery

### 3.1 Retry policy
Infrastructure failures (OOM, CUDA init, file missing, network blip) are retried at 30s, 5min, 30min backoffs. After the third failure the row is marked `failed` and the runner moves on.

### 3.2 OOM handling
- Catch `torch.cuda.OutOfMemoryError`.
- On first OOM: halve the batch size for this run, log the change, retry.
- On second OOM at halved batch: halve again.
- On third OOM: mark `failed`, move on.

### 3.3 CUDA error handling
- Capture stderr. If it contains `CUDA error` or `cublas` or `NCCL`, run `nvidia-smi` and log output, then retry.
- If `nvidia-smi` itself fails, mark the run `failed` and move on without retry.

### 3.4 W&B outage handling
- Runs always write to `results.tsv` first, then attempt W&B upload with a 30-second timeout.
- W&B failures do not mark the experiment as failed. They log a warning and queue a retry in `data/wandb_retry_queue.json` for a sweep.

### 3.5 Disk full
- Before each run, check free space on `/` and on the NVMe holding `data/`. If either is under 50 GB free, mark the run `failed` with reason `disk_full`, do not retry.

## 4. Pause / resume

The runner polls for sentinel files between rows.
- Graceful pause: `touch data/PAUSE`. Runner finishes the current experiment, writes results, commits, then waits for `data/PAUSE` to be removed before picking up the next row.
- Resume: `rm data/PAUSE`.
- Emergency stop: `touch data/HALT`. Runner exits cleanly at the next loop iteration after writing state.
...
## 6. Experiment-specific rules

### 6.1 Anchor halt gate (Experiment 3)
...
## 7. What the runner must never do

- Modify `EXPERIMENTS.yaml`, `DECISIONS.md`, `SPEC.md`, or `README.md`.
